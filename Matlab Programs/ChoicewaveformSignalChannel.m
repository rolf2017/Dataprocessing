%% Signal Channel Choice waveform
%打开选用数据的文件，寻找想要提取的图像
%x输入起始数据点，y输入终止数据点；
%绘制xy之间数据时间域图像
%% 
clc;
clear all;
close all;
%%
load 'D:\WorkMatlab\201612\2M-1ch-10V-EME-test7.mat' %文件绝对路径
a=d_unnamedTask2PXI1Slot3ai1.Data/100;%注意更改通道任务名称
fs=2000000;N=length(a);n=0:N-1;t=0:1/fs:(N-1)/fs;
x=1;%起始数据点
y=20000000;%终止数据点
a1=a(x:y,:);
Hd = lowpass800kHz;%800kHz低通滤波器
h1=filter(Hd,a1);%滤波
f=(0:(y-x))*fs/length(a1)/1000;
a2=abs(fft(h1));
figure(1)
% subplot(211); 
plot(t(x:y),a(x:y)*1000,'k','linewidth',1);
ylim([-1,1])%纵坐标
title('EME','fontsize',14);
ylabel('幅值/mV','fontsize',14);
xlabel('时间/s','fontsize',14);
set(gca,'fontsize',14);
% subplot(212);
% plot(f(2:floor((y-x)/2)),a2(2:floor((y-x)/2)),'k');
% xlim([0,800]);
% title('FFT','fontsize',14);
% ylabel('FFT','fontsize',14);
% xlabel('f/kHz','fontsize',14);
 %set(gca,'fontsize',14,'position',[0.10 0.14 0.89 0.75]);%图长宽高

   % set(gcf,'position',[384   280  800   500])%窗口大小
    %print(gcf,'-dmeta','D:\2016doc\docpicture\test')%输出路径
    %print(gcf,'-deps','D:\2016doc\docpicture\test')%输出路径
   % saveas(gcf,'D:\WorkMatlab\201612\plotdata','fig')%保存路径